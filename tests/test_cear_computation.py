...
class CearTestCase(TransactionCase):
    ...
    def test_commitment(self):
        cear_a = self.create_cear('Commitment A', 15000.0, 0.0, False)
        cear_b = self.create_cear('Commitment B', 10000.0, 0.0, False)
        self.assertEqual(cear_a.commitment_amount, 15000.0)
        self.assertEqual(cear_b.commitment_amount, 10000.0)

        cear_c = self.env['budget.capex.cear'].create({
            'input_no': 'Distributed A',
            'has_distribution': True,
            'distribution_ids': [(0, False, {'input_commitment_amount': 1000.0, 'parent_id': cear_a.id}),
                                 (0, False, {'input_commitment_amount': 2000.0, 'parent_id': cear_b.id})
                                 ]})

        self.assertEqual(cear_c.commitment_amount, 3000.0)

    def test_expenditure(self):
        cear_a = self.create_cear('Expenditure A', 0.0, 15000.0, False)
        cear_b = self.create_cear('Expenditure B', 0.0, 10000.0, False)
        self.assertEqual(cear_a.expenditure_amount, 15000.0)
        self.assertEqual(cear_b.expenditure_amount, 10000.0)

        cear_c = self.env['budget.capex.cear'].create({
            'input_no': 'Distributed A',
            'has_distribution': True,
            'distribution_ids': [(0, False, {'input_expenditure_amount': 1000.0, 'parent_id': cear_a.id}),
                                 (0, False, {'input_expenditure_amount': 2000.0, 'parent_id': cear_b.id})
                                 ]})

        self.assertEqual(cear_c.expenditure_amount, 3000.0)
